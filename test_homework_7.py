...
def test_create_zip():
    ...
    assert "zip.zip" in file_list, f"file is not created in resourses"
...
def test_read_csv_file():
    ...
    assert text == b'Privet', f"Privet us not in csv"
...
def test_read_pdf_file():
    with ZipFile('resourses/zip.zip') as zip_file:
        with zip_file.open('pdf.pdf') as pdf:
            ...
            assert "PDF" in reader.pages[0].extract_text(), f"PDF is not is pdf"
...
def test_read_xlsx_file():
    with ZipFile("resourses/zip.zip") as zip_file:
        with zip_file.open('xlsx.xlsx') as xlsx:
            ...
            assert a == 'Privet', f"Privet is not in xlsx"